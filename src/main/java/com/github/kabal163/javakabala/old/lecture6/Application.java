...
//        runCheckedExceptionExample(); // мы видим, что нам нужно либо обработать исключение, либо пробросить его дальше.
        ...
        } catch (Exception ex) { // тут объявляются исключения, которые мы хотим перехватывать
            System.out.println("Мы перехватили исключение: " + ex.getMessage()); // обрабатываем исключение. Тут можно делать что-то более осознанное. Но в данном случае мы просто печатаем stacktrace
        }
        System.out.println("----------------");
        ...
        try {
            ...
            System.out.println("Любые другие ошибки");
        }  finally {
